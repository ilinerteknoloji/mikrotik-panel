import { mysqlTable, mysqlSchema, AnyMySqlColumn, foreignKey, primaryKey, int, text, tinyint, timestamp, unique, varchar, mysqlEnum } from "drizzle-orm/mysql-core"
...
export const mikrotik_ips = mysqlTable("mikrotik_ips", {
	id: int("id").autoincrement().notNull(),
	user_id: int("user_id").notNull().references(() => users.id),
	ip: text("ip").notNull(),
	status: tinyint("status").default(1),
...
(table) => {
	return {
		mikrotik_ips_id: primaryKey({ columns: [table.id], name: "mikrotik_ips_id"}),
	}
});
